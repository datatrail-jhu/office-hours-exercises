...
Great! That narrowed this down to 108 restaurants. Let's try to plot it again.

```{r}
ggplot(top_num_restaurants, aes(name, n)) + 
  geom_bar(stat = "identity")
```
...
Let's add a layer that will turn the text and make it more legible.
...
ggsave(file.path("results", "restaurant_barplot.png"))
```
